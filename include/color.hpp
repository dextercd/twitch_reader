...
constexpr std::uint8_t hex_char_to_value(char c)
{
...
public:
  ...
  constexpr void assign(std::uint8_t r, std::uint8_t g, std::uint8_t b)
  {
    rgb[0] = r;
    rgb[1] = g;
    rgb[2] = b;
...
inline void set_foreground_color(std::ostream& os, color c)
{
  os << "\x1b[38;2;" << (unsigned)c.rgb[0] << ';' << (unsigned)c.rgb[1] << ';' << (unsigned)c.rgb[2]
     << 'm';
}
inline void set_background_color(std::ostream& os, color c)
{
  os << "\x1b[48;2;" << (unsigned)c.rgb[0] << ';' << (unsigned)c.rgb[1] << ';' << (unsigned)c.rgb[2]
     << 'm';
}

inline void reset_color(std::ostream& os)
{
  os << "\x1b[0m";
}
